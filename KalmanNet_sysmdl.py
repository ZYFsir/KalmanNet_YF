...
class EightCurv():
    ...
    def __init__(self, x,y,theta):
        self.t = np.linspace(-np.pi, np.pi, 1000)

    def F(self):
        t = self.t
        a = 10
        self.x = a*np.cos(t)
        self.y = a*np.sin(t)*np.cos(t)

    def plot(self):
        plot(self.x, self.y)
        show()

    def getTanAngle(self):
        thi = [np.arctan(np.sqrt(7)-4*np.cos(self.t))]

class MovSeq():
    def __init__(self, modelSeq, x, y, theta):
